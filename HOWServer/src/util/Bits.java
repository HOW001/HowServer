...
public class Bits {
	 ...
	 * @param b
	 * @param off
	 * @param length  -1 代表  off 以后直到b结尾
	 * @return
	 */
	public static byte[] getByte(byte[] b,int off,int length){
		if(length==-1){
			length=b.length-off;
		}		
		byte []temp=new byte[length];
		System.arraycopy(b,off,temp,0,length);
		return temp;
	}
	...
	public static void putBytes(byte[] b,int off,byte[] src){
		long len1 = b.length;
		if(off > len1){
			System.err.println("偏移量" + off + "超过了目标数组的长度:" + len1);
			return;
		}
		long len2 = src.length + off;
		if(len2 > len1){
			System.err.println("源数组的长度和偏移量的和" + len2 + "超过了目标数组的长度:" + len1);
			return;
		}
